...
*/
	class Bannerpage extends MySql{
			...
			*/			
			function updateNews($inputs){
					$insert	=	array('title'=>$this->addFilter($this->escapeHtml($inputs['title'])),											
											...
											'status'=>$inputs['published']);
				$this->update($insert,"cms_banner",'`banner_id`='.$inputs['id']);		
				return true;
			}
			...
			*/
			...
			function listNews($values){
					...
					$query	=	"SELECT count(c.`banner_id`) FROM `cms_banner` c WHERE c.`banner_id`!=''";
					$query.=$qry;
					$rec	=	$this->fetchAll($query);
					$this->totalRecords	=	$rec[0]['count(c.`banner_id`)'];
		$query	=	"SELECT c.* FROM `cms_banner` c WHERE c.`banner_id`!=''";
					$query.=$qry;
					$query.=$order;
  	...
  	$destinationPath="../uploads/";
			...
			function deleteList($list){
					for($i=0;$i<count($list);$i++){
						$this->dltimg($list[$i]);
							$this->delete('cms_banner','`banner_id`='.$list[$i]);
					}
			}
			...
			*/	
			function unpublishList($list){
				for($i=0;$i<count($list);$i++){
					$this->update(array('status'=>'0'),"cms_banner",'`banner_id`='.$list[$i]);
				}
			}
			...
			*/
			...
			function publishList($list){
				for($i=0;$i<count($list);$i++){
					$this->update(array('status'=>'1'),"cms_banner",'`banner_id`='.$list[$i]);
				}
			}
