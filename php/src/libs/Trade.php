...
*/
	class Trade extends MySql{
			...
			*/			
			function updateNews($inputs){
				...
				$this->update($insert,"cms_trade",'`id`='.$inputs['id']);		
				return true;
			}
			...
			*/
			...
			function listNews($values){
					...
					$query	=	"SELECT count(c.`id`) FROM `cms_trade` c WHERE c.`id`!=''";
					$query.=$qry;
					$rec	=	$this->fetchAll($query);
					$this->totalRecords	=	$rec[0]['count(c.`id`)'];
		$query	=	"SELECT c.* FROM `cms_trade` c WHERE c.`id`!=''";
					$query.=$qry;
					$query.=$order;
 ...
 $query	=	'SELECT * FROM `cms_trade` WHERE `id`='.$id.' ';
		$rec	=	$this->fetchAll($query);
  ...
  function deleteUp($image){
  	$destinationPath="../uploads/";
			...
			*/
			...
			function getl(){
$id=1;
				$query	=	'SELECT * FROM `cms_trade` WHERE `published`='.$id;
				$rec		=	$this->fetchAll($query);
			...
			*/
			...
			function deleteList($list){
					for($i=0;$i<count($list);$i++){
						$this->dltimg($list[$i]);
							$this->delete('cms_trade','`id`='.$list[$i]);
					}
			}
			...
			*/	
			function unpublishList($list){
				for($i=0;$i<count($list);$i++){
					$this->update(array('published'=>'0'),"cms_trade",'`id`='.$list[$i]);
				}
			}
			...
			*/
			...
			function publishList($list){
				for($i=0;$i<count($list);$i++){
					$this->update(array('published'=>'1'),"cms_trade",'`id`='.$list[$i]);
				}
			}
