...
*/
	class Testimony extends MySql{
			...
			*/			
			function addNews($inputs){
				...
				$this->insert($insert,"cms_testimony");		
				return true;
			}
			...
			*/			
			function updateNews($inputs){
				...
				$this->update($insert,"cms_testimony",'`testimony_id`='.$inputs['id']);		
				return true;
			}
			...
			*/
			...
			function listNews($values){
					...
					$query	=	"SELECT count(c.`testimony_id`) FROM `cms_testimony` c WHERE c.`testimony_id`!=''";
					$query.=$qry;
					$rec	=	$this->fetchAll($query);
					$this->totalRecords	=	$rec[0]['count(c.`testimony_id`)'];
		$query	=	"SELECT c.* FROM `cms_testimony` c WHERE c.`testimony_id`!=''";
					$query.=$qry;
					$query.=$order;
 ...
 $query	=	'SELECT * FROM `cms_testimony` WHERE `testimony_id`='.$id.' ';
		$rec	=	$this->fetchAll($query);

$this->deleteUp($rec[0]['image']);
return true;
  ...
  function deleteUp($image){
  	$destinationPath="../uploads/";
			...
			*/
			...
			function getl(){
$id=1;
				$query	=	'SELECT * FROM `cms_testimony` WHERE `published`='.$id;
				$rec		=	$this->fetchAll($query);
			...
			*/
			...
			function deleteList($list){
					for($i=0;$i<count($list);$i++){
						$this->dltimg($list[$i]);
							$this->delete('cms_testimony','`testimony_id`='.$list[$i]);
					}
			}
			...
			*/	
			function unpublishList($list){
				for($i=0;$i<count($list);$i++){
					$this->update(array('published'=>'0'),"cms_testimony",'`testimony_id`='.$list[$i]);
				}
			}
			...
			*/
			...
			function publishList($list){
				for($i=0;$i<count($list);$i++){
					$this->update(array('published'=>'1'),"cms_testimony",'`testimony_id`='.$list[$i]);
				}
			}
