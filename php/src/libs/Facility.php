...
function addimage($imagename,$imageloc){
				...
				$this->insert($insert,"cms_facility");		
				return true;
			}
	function updateimagetitle($id,$name){
				
					$this->update(array('description'=>$name),"cms_facility",'`id`='.$id);
			...
			*/
			...
			function deleteList($list){
					for($i=0;$i<count($list);$i++){
							$this->dltlist($list[$i]);
							$this->delete('cms_facility','`id`='.$list[$i]);
					}
			}
function dltlist($id){

$query	=	'SELECT * FROM `cms_facility` WHERE `id`='.$id.' ';
  ...
  function deleteUp($image){
  	$destinationPath="../uploads/";
		...
		function getall(){
			$id=1;
		$query	=	'SELECT * FROM `cms_facility` WHERE `status`='.$id.' ';
		$rec	=	$this->fetchAll($query);
		return $rec;
	}
/*
			...
			*/	
			function unpublishList($list){
				for($i=0;$i<count($list);$i++){
					$this->update(array('status'=>'0'),"cms_facility",'`id`='.$list[$i]);
				}
			}
			
				function getmax(){

				$query	=	'SELECT MAX(`id`) FROM `cms_facility` ';
				$rec		=	$this->fetchAll($query);
				return $rec;
			}
			/*
			...
			*/
			...
			function publishList($list){
				for($i=0;$i<count($list);$i++){
					$this->update(array('status'=>'1'),"cms_facility",'`id`='.$list[$i]);
				}
			}
	function listimage($values,$passid){
		...
		$query	=	"SELECT count(c.`id`) FROM `cms_facility` c ";
		 $query.=$qry;
		$rec	=	$this->fetchAll($query);
		$this->totalRecords	=	$rec[0]['count(c.`id`)'];
		
		$query	=	"SELECT * FROM `cms_facility` c ";
		$query.=$qry;
		$query.=$order;
