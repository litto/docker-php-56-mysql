...
$id=1;
			...
			*/			
			function updateNews($inputs){
				...
				$this->update($insert,"cms_news",'`news_id`='.$inputs['id']);		
				return true;
			}
  	...
  	$destinationPath="../uploads/";
			...
			*/
			...
			function deleteList($list){
					for($i=0;$i<count($list);$i++){
						$this->dltimg($list[$i]);
							$this->delete('cms_news','`news_id`='.$list[$i]);
			...
			*/	
			function unpublishList($list){
				for($i=0;$i<count($list);$i++){
					$this->update(array('published'=>'0'),"cms_news",'`news_id`='.$list[$i]);
				}
			}
			...
			*/
			...
			function publishList($list){
				for($i=0;$i<count($list);$i++){
					$this->update(array('published'=>'1'),"cms_news",'`news_id`='.$list[$i]);
				}
			}
